/**
 * 
 * 
 * Alien Colors #2: Choose a color for an alien as you did in Exercise 25, and
write an if-else chain.
• If the alien’s color is green, print a statement that the player just earned 5 points for shooting the
alien.
• If the alien’s color isn’t green, print a statement that the player just earned 10 points.
• Write one version of this program that runs the if block and another that runs the else block.
 * 
 * 
 */

let alienc:string = "green";
let alien1:string= "yellow";
if(alienc=== "green"){
    console.log("played earned 5 points");

}
if(alien1 !=="green"){
    console.log("played earned 10 points");

}

if(alienc=== "green"){
    console.log("the colour of alien in green");

}else{
    console.log("the colouer of alien is yellow")
}
if(alien1=== "green"){
    console.log(" ");

}else{
    console.log("the colouer of alien is yellow")
}

